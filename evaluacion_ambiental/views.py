...
def data_scraper(
    ...
    if homepage >= final_page:
        return Response({"error": "final_page debe ser mayor que homepage"},status.HTTP_400_BAD_REQUEST)
    elif final_page > pages_max:
        return Response({"error": f"final_page debe ser menor a {pages_max}"},status.HTTP_400_BAD_REQUEST)
    else:
        data_file, page, last_page = data_injection_from_scraper(homepage , final_page)
        if int(last_page) > homepage:
            return Response({"error": f"La ultima pagina scrapeada fue pag. {int(last_page)}"},status.HTTP_400_BAD_REQUEST)
        if page != final_page:
            return Response({"message": f"Se detuvo el scrapeo en la pagina {page}"}, status.HTTP_200_OK)
